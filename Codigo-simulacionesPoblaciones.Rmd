...
* Un vector $\texttt{p_vec}$ del mismo largo que la cantidad de individuos considerados. Este vector tendrá en la entrada $i$-ésima la proporción de datos faltantes que se construirán para el individuo $i$-ésimo. Por esta razón, cada una de las entradas de $\texttt{p_vec}$ debe ser un valor numérico entre $0$ y $1$.
...
hacerNAindividuo <- function(M, indCol, p_vec, p){
	...
	if(indCol == T){
		...
		for (i in 1:m){
			cantNA <- floor(m * p_vec[i])
			vecNA <- sample(x = 1:n, size = cantNA, replace = FALSE)
...
La función implementada es $\texttt{limpiarMatriz()}$. Esta toma como parámetro de entrada una matriz $\mathbf{M} \in \R^{m \times n}$ cuyas columnas corresponden a observaciones y las filas a variables y devuelve la matriz $\tilde{\M} \in \R^{m \times q}$ con $q \leq n$ sin aquellas filas que no son útiles. Será necesario también establecer si los individuos son las filas o las columnas de la matriz; esto está determinado por el parámetro $\texttt{indCol}$ que tomará el valor $\texttt{TRUE}$ si los individuos son vectores columnas y $\texttt{FALSE}$ si no. En caso de que hayan sido borradas algunas filas, la función también devolverá un vector, $\texttt{indices}$, con el índice de las filas que fueron borradas.
...
simulaPob <- function(k, n, d, metodo, p, theta, p_vec){
	...
	return(list("matriz_original" = M, "matriz_na" = mLimp$Resultado, "indices" = mLimp$indices))
}
```
